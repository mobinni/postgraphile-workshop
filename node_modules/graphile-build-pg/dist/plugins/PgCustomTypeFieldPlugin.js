        ...
        hooks: {
            build: {
                callback(build) {
                    ...
                    const { graphql: { GraphQLList, GraphQLNonNull, isInputType }, } = build;
                    build.pgGetArgDetailsFromParameters = (resource, parameters = resource.parameters) => {
                        const argDetails = parameters.map((param, index) => {
                            ...
                            const baseInputType = build.getGraphQLTypeByPgCodec(paramBaseCodec, variant);
                            if (!baseInputType) {
                        ...
                        const argDetailsSimple = argDetails.map(({ graphqlArgName, pgCodec, required, postgresArgName }) => ({
                            graphqlArgName,
                            postgresArgName,
                            pgCodec,
                            required,
                        ...
                        const makeArgs = (0, graphile_build_1.EXPORTABLE)((argDetailsLength, argDetailsSimple, constant, indexAfterWhichAllArgsAreNamed) => (args, path = []) => {
                            const selectArgs = [];
                            let skipped = false;
                            for (let i = 0; i < argDetailsLength; i++) {
                                const { graphqlArgName, postgresArgName, pgCodec, required, } = argDetailsSimple[i];
            ...
            init: {
                after: ["PgCodecs"],
                callback(_, build) {
                    const { graphql: { GraphQLList, GraphQLString }, inflection, options, pgGetArgDetailsFromParameters, } = build;
                        ...
                        build.recoverable(null, () => {
                            ...
                            if (isMutationProcSource) {
                                ...
                                build.recoverable(null, () => {
                                    ...
                                    const returnGraphQLTypeName = build.getGraphQLTypeNameByPgCodec(resource.codec.arrayOfCodec ?? resource.codec, "output");
                                    const resultFieldName = isVoid || !returnGraphQLTypeName
                                        ? null
            ...
            GraphQLObjectType_fields(fields, build, context) {
                ...
                const { Self, scope: { isPgClassType, pgCodec, isRootQuery, isRootMutation }, fieldWithHooks, } = context;
                ...
                return procSources.reduce((memo, resource) => build.recoverable(memo, () => {
                    ...
                    const getSelectPlanFromParentAndArgs = isRootQuery
                        ...
                        : isRootMutation
                            ...
                            : // Otherwise computed:
                                (0, graphile_build_1.EXPORTABLE)((PgSelectSingleStep, hasRecord, makeArgs, pgClassExpression, pgSelectSingleFromRecord, resource, stepAMayDependOnStepB) => ($in, args, _info) => {
                                    if (!hasRecord($in)) {
                                    ...
                                    if (resource.isUnique &&
                                        ...
                                        return pgClassExpression($row, resource.codec) `${resource.from(...placeholders.map((placeholder) => ({
                                            placeholder,
                                        })))}`;
                                    }
                    ...
                    if (isRootMutation) {
                        ...
                        memo[fieldName] = fieldWithHooks({ fieldName, fieldBehaviorScope: "mutationField" }, {
                            ...
                            type: build.nullableIf(!resource.extensions?.tags?.notNull, payloadType),
                            args: {
                                input: {
