    ...
    gather: {
        ...
        helpers: {
            async getIntrospectionData(info, serviceName, pgClass, attributes) {
                ...
                try {
                    const { rows } = await (0, pgServices_js_1.withPgClientFromPgService)(pgService, null, (client) => client.query(query));
                    return rows;
                }
                catch (e) {
                    let role = "RELEVANT_POSTGRES_USER";
                    try {
                        const { rows } = await (0, pgServices_js_1.withPgClientFromPgService)(pgService, null, (client) => client.query({
                            text: "select user;",
                        }));
                        if (rows[0]) {
                            role = rows[0].user;
            ...
            async processIntrospection(info, event) {
                ...
                for (const pgClass of introspection.classes) {
                    ...
                    if (isEnumTable) {
                        ...
                        (0, utils_js_1.addBehaviorToTags)(tags, "-*", true);
                    }
