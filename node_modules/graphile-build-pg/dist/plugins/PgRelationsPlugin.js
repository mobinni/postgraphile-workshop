...
function makeSpecString(identifier, localAttributes, remoteAttributes) {
    return (0, tamedevil_1.default) `{ ${tamedevil_1.default.join(remoteAttributes.map((remoteAttributeName, i) => (0, tamedevil_1.default) `${tamedevil_1.default.dangerousKey(remoteAttributeName)}: ${identifier}.get(${tamedevil_1.default.lit(localAttributes[i])})`), ", ")} }`;
}
...
function addRelations(fields, build, context) {
    ...
    if (!(isPgClassType || isMutationPayload || pgPolymorphism) ||
        !codec ||
        !codec.attributes) {
        return fields;
    }
    ...
    const refDefinitionList = isMutationPayload
        ...
        : codec.refs
            ...
            : Object.entries(codec.extensions?.refDefinitions ??
                Object.create(null)).map(([refName, refDefinition]) => ({
                refName,
                refDefinition,
                codec,
            }));
    ...
    if (relations) {
        ...
        for (const [relationName, relation] of Object.entries(relations)) {
            ...
            const shouldAddSingleField = build.behavior.pgCodecRelationMatches(relation, `${relationTypeScope}:resource:single`);
    ...
    conditions: [
      ${tamedevil_1.default.join(remoteAttributes.map((remoteAttrName, i) => {
                    ...
                    const makePlanResolver = (mode) => {
                        ...
                        return (0, graphile_build_1.EXPORTABLE)((attributes, connection, isConnection, isMutationPayload, members, paths, pgUnionAll, resourceByTypeName, single) => ($parent) => {
        ...
        const relationTypeScope = isUnique ? `singularRelation` : `manyRelation`;
        const shouldAddSingleField = build.behavior.stringMatches(behavior, `${relationTypeScope}:resource:single`);
    ...
    return digests.reduce((memo, digest) => {
        const { isUnique, relationTypeScope, shouldAddSingleField, shouldAddConnectionField, shouldAddListField, typeName, connectionTypeName, deprecationReason, singleRecordFieldName, listFieldName, connectionFieldName, singleRecordPlan, listPlan, connectionPlan, isReferencee, identifier, description, pgResource: pgFieldResource, pgCodec: pgFieldCodec, pgRelationDetails, relatedTypeName, } = digest;
        ...
        if (isUnique && shouldAddSingleField) {
            ...
            }, `Adding '${identifier}' single relation field to ${Self.name}`, "recoverable");
        }
