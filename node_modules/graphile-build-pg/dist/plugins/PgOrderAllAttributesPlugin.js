        ...
        hooks: {
            GraphQLEnumType_values(values, build, context) {
                ...
                return extend(values, Object.entries(attributes).reduce((memo, [attributeName, attribute]) => {
                    if (!build.behavior.pgCodecAttributeMatches([pgCodec, attribute], "attribute:orderBy")) {
                        return memo;
                    }
                    if (attribute.codec.arrayOfCodec) {
                        if (!build.behavior.pgCodecAttributeMatches([pgCodec, attribute], "attribute:orderBy:array")) {
                            return memo;
                        }
                    }
                    if (attribute.codec.rangeOfCodec) {
                        if (!build.behavior.pgCodecAttributeMatches([pgCodec, attribute], "attribute:orderBy:range")) {
                            return memo;
                        }
                    }
                    ...
                    memo = extend(memo, {
                        [ascFieldName]: {
                            extensions: {
                                grafast: {
                                    applyPlan: (0, graphile_build_1.EXPORTABLE)((PgSelectStep, PgUnionAllStep, attributeName, isUnique, pgOrderByNullsLast) => (plan) => {
