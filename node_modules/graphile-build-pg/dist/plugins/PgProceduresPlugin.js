            ...
            async getResourceOptions(info, serviceName, pgProc) {
                ...
                resourceOptionsPromise = (async () => {
                    ...
                    const makeCodecFromReturn = async () => {
                        ...
                        return (0, graphile_build_1.EXPORTABLE)((attributes, recordCodec, recordCodecName, sql) => recordCodec({
                            ...
                            isAnonymous: true,
                        }), [attributes, pg_1.recordCodec, recordCodecName, pg_sql2_1.default]);
                    };
                    ...
                    const numberOfRequiredArguments = numberOfArguments - numberOfArgumentsWithDefaults;
                    ...
                    for (let i = 0, l = numberOfArguments; i < l; i++) {
                        ...
                        if (argMode === "i" || argMode === "b") {
                            ...
                            const argCodec = await info.helpers.pgCodecs.getCodecFromType(serviceName, argType, undefined);
                            if (!argCodec) {
                                console.warn(`Could not make codec for '${debugProcName}' argument '${argName}' which has type ${argType} (${(await info.helpers.pgIntrospection.getType(serviceName, argType)).typname}); skipping function`);
                                return null;
                            }
                            const required = i < numberOfRequiredArguments;
                            const notNull = isStrict || (isStrictish && i < numberOfRequiredArguments);
                            /*
                    ...
                    if (!returnCodec.isAnonymous &&
                        ...
                        const resourceConfig = await (async () => {
                            if (resourceOptions) {
                                ...
                                const executor = info.helpers.pgIntrospection.getExecutorForService(serviceName);
                                return pg_1.PgResource.configFromCodec(executor, codec);
                            }
                        })();
                        if (!resourceConfig) {
                            return null;
